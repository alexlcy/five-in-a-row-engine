...
class AllPatternEncoder(Encoder):
    ...
    def encode(self, mat, cur_player, last_move):
        ...
        layers_break_down = potential_open_moves(mat)
        layer_last_move = self.encode_point(last_move)
        final_X = np.dstack([layer_my_move, layer_oppo_move, layer_last_move, layer_valid_move] + layers_break_down)
...
def create(board_size):
    return AllPatternEncoder(board_size)
...
def break_down_layers(game_state):
    ...
    for search_num in reversed(range(1,5)):
